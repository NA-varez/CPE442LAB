...
void to442_sobel(Mat& input, Mat& output) {
	...
	for (int i = 1; i < input.rows - 1; ++i) {
		for (int j = 1; j < input.cols - 1; ++j) {
			...
			uchar g_x = (-1*input.at<uchar>(i-1,j-1)) + input.at<uchar>(i-1,j+1) +
					(-2*input.at<uchar>(i,j-1)) + 2*input.at<uchar>(i,j+1) +
					(-1*input.at<uchar>(i+1,j-1)) + input.at<uchar>(i+1,j+1);
			uchar g_y = (-1*input.at<uchar>(i-1,j-1)) + -1*input.at<uchar>(i-1,j+1) +
			...
			output.at<uchar>(i, j) = std::abs(g_x) + std::abs(g_y);
		}
